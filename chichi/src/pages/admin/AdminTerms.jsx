...
function MarkdownPreview({ text }) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('# ')) {
      ...
      elements.push(<li key={key++} className="ml-5 list-disc font-body text-sm text-muted leading-relaxed">{line.slice(2).replace(/\*\*(.+?)\*\*/g, (_, m) => m)}</li>)
    } else if (line.trim() === '') {
      ...
      elements.push(
        <p key={key++} className="font-body text-sm text-muted leading-relaxed">
          {parts.map((part, idx) => idx % 2 === 1 ? <strong key={idx} className="font-semibold text-espresso">{part}</strong> : part)}
        </p>
      )
    }
  }
...
export default function AdminTerms() {
  const { termsContent, updateTerms } = useApp()
  const [draft, setDraft] = useState(termsContent)
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="font-body text-sm text-muted">Edit the T&amp;C shown to sellers in their dashboard.</p>
        </div>
