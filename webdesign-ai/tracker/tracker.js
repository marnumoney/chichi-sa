...
function send(payload){
  ...
  // Use fetch+keepalive instead of sendBeacon — sendBeacon sends credentials which breaks wildcard CORS.
  // keepalive ensures the request completes even if the page is unloading.
  fetch(AGENT+'/api/agent/ingest',{method:'POST',headers:{'Content-Type':'application/json'},body,keepalive:true}).catch(()=>{});
}
